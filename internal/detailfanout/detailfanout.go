...
// HasPlan reports whether Plan would return any work, without building the
// slice. A caller that only needs the yes/no — deciding whether to offer a key,
// say — pays one pass and no allocation.
func HasPlan(tickets []model.Ticket, have func(model.TicketID) bool) bool {
	for _, t := range tickets {
		if t.ID == "" {
			continue
		}
...
// detailFailures splits a FetchDetails error into per-Ticket failures and a
// response-wide remainder. Provider is a closed, compiled-in interface, so the
// aggregate it returns is a trusted in-tree value: a plain errors.As is enough.
