...
OSC 8 hyperlinks emitted outside explicit Link rows—such as Ticket keys, header URLs, Trail breadcrumbs, and pull-request summaries—remain terminal hyperlinks only. They are never inferred as Ticket relationships and never create Trail entries. URLs in descriptions, comments, raw markdown, and arbitrary body text are not promoted by this decision; if a future renderer emits them as hyperlinks, adding in-app body or markdown navigation would still require a separate decision rather than string-scanning rendered output.
